...
def build_dataset(
    ...
    for _, v in folders.items():
        ...
        dataset = AudioDataset(loader, sample_rate, duration=duration, offset=1.0, transform=transform)
...
def load(
    ...
    with argbind.scope(args, "generator"):
        params_to_update = list(generator.encoder.parameters()) + list(generator.decoder.parameters()) + list(generator.wavlm_projection.parameters())
...
def get_units(batch):
    ...
    target_output_wavlm = torch.zeros(b, 768, 64)
    ...
    for i, p in enumerate(batch["path"]):
        ...
        output_wavlm = unit_dict[p]['wavlm_output']
        
        unit_hubert = unit_hubert[start:start+unit_length].unsqueeze(0)
        unit_wavlm = unit_wavlm[start:start+unit_length].unsqueeze(0)
        
        output_wavlm = output_wavlm[start:start+unit_length, :].unsqueeze(0)
        output_wavlm = torch.nn.functional.avg_pool1d(output_wavlm.transpose(2,1), kernel_size=11, stride=1)
        
        target_units_hubert[i, :] = unit_hubert
        target_units_wavlm[i, :] = unit_wavlm
        target_output_wavlm[i, :] = output_wavlm
    
    return target_units_hubert, target_units_wavlm, target_output_wavlm
...
def train_loop(state, batch, accel, lambdas, update_disc_every, warmup):
    ...
    with torch.no_grad():
        ...
        target_units_hubert, target_units_wavlm, target_output_wavlm = get_units(batch)
    ...
    with accel.autocast():
        ...
        print(projected_z_wavlm.shape, target_output_wavlm.shape)

        loss_wavlm = state.cos_loss(projected_z_wavlm, target_output_wavlm.to(recons.device))
    ...
    with accel.autocast():
        ...
        output["gen/cos_loss"] = loss_wavlm
        if state.warmed_up:
