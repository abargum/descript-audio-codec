...
def get_audio(batch, state, accel):
    ...
    out = state.generator(signal.audio_data)
    
    recons = AudioSignal(out["audio"], signal.sample_rate)
    sine = AudioSignal(out["pitch_audio"], signal.sample_rate)
    inp = AudioSignal(signal.audio_data, signal.sample_rate)

    return inp, recons, sine, signal.sample_rate
...
def train_loop(state, batch, accel, lambdas, update_disc_every, warmup):
    ...
    with accel.autocast():
        ...
        pitch_signal = AudioSignal(out["pitch_audio"], signal.sample_rate)
        sine_target = AudioSignal(out["sine_target"], signal.sample_rate)
    ...
    with accel.autocast():
        ...
        output["gen/ex_loss"] = torch.nn.functional.mse_loss(pitch_signal.audio_data, sine_target.audio_data)
        if state.warmed_up:
...
def validate(state, val_dataloader, accel):
    ...
    inp, recon, sine, sr = get_audio(last_batch, state, accel)
    inp = inp.audio_data.float()
    recon = recon.audio_data.float()
    sine = sine.audio_data.float()

    audio = torch.cat([inp, recon, sine], -1)
