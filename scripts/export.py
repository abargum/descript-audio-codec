...
class Scripted(nn_tilde.Module):
    ...
    def forward(self, inputs: Tuple[torch.Tensor, torch.Tensor, torch.Tensor]):
        ...
        loudness = extract_loudness(x, sr=self.sr, block_size=256)
...
def main():
    ...
    pitch_encoder = Encoder(data_size = 1,
                            capacity = 32,
                            ratios = [8, 4, 4, 2],
                            latent_size = 1440,
                            n_out = 1,
    ...
    pitch_encoder.load_state_dict(torch.load(f"scripts/utils/16_no_pqmf.pth", weights_only=True))
    pitch_encoder.eval()
    ...
    emb = generator.speaker_encoder(emb_audio).unsqueeze(-1)
