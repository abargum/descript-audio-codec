...
class BatchRandomNoise(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        noise_std = snr * x.reshape(batch_size, -1).std(dim=-1)
        noise_std = noise_std.unsqueeze(-1).expand_as(x.reshape(batch_size, -1)).view_as(x)
...
class BatchRandomGain(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        vol[mask] = 1
        vol = vol.unsqueeze(-1).expand_as(x.reshape(batch_size, -1)).view_as(x)
        return vol * x