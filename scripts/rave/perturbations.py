...
#Scripts from https://github.com/dhchoi99/NANSY
...
def wav_to_Sound(wav, sampling_frequency: int = 48000) -> parselmouth.Sound:
...
def get_pitch_median(wav, sr: int = None):
    ...
    try:
        ...
        pitch_median = parselmouth.praat.call(pitch, "Get quantile", 0.0, 0.0,
                                              0.5, "Hertz")
...
def change_gender(
    ...
    try:
        if pitch is None:
            new_sound = parselmouth.praat.call(sound, "Change gender", 75, 600,
                                               formant_shift_ratio,
                                               new_pitch_median,
                                               pitch_range_ratio,
                                               duration_factor)
            ...
            new_sound = parselmouth.praat.call(
                (sound, pitch), "Change gender", formant_shift_ratio,
...
def apply_formant_and_pitch_shift(
    ...
    if pitch_shift_ratio != 1.:
        try:
            ...
            pitch_minimum = parselmouth.praat.call(pitch, "Get minimum", 0.0,
                                                   0.0, "Hertz", "Parabolic")
            newMedian = pitch_median * pitch_shift_ratio
            scaledMinimum = pitch_minimum * pitch_shift_ratio
            resultingMinimum = newMedian + (scaledMinimum -
                                            newMedian) * pitch_range_ratio
    ...
    new_sound = change_gender(sound, pitch, formant_shift_ratio,
                              new_pitch_median, pitch_range_ratio,
                              duration_factor)
...
def parametric_equalizer(wav, sr: int):
    ...
    key_freqs = [
        power_ratio(
            float(z) / (num_filters), cutoff_low_freq, cutoff_high_freq)
...
def apply_iir_filter(wav,
    ...
    if ftype == 'low':
        ...
        b0, b1, b2, a0, a1, a2 = highShelf_coeffs(dBgain, cutoff_freq,
                                                  sample_rate, Q)
    elif ftype == 'peak':
        b0, b1, b2, a0, a1, a2 = peaking_coeffs(dBgain, cutoff_freq,
                                                sample_rate, Q)
    else:
        raise NotImplementedError
    if torch_backend:
...
def perturb_pitch_and_formant(wav: torch.Tensor, sr: int) -> torch.Tensor:
    wav_numpy = wav.numpy()
